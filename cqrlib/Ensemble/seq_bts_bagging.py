...
def _if_delegate_has_method(delegate):
    """Replacement for sklearn.utils.metaestimators.if_delegate_has_method."""
...
from cqrlib.Sampling.sample_unique import mp_seq_bts, mp_idx_matrix, av_unique
...
def _generate_bagging_indices(
    random_state, bootstrap_features, bootstrap_samples, n_features, n_samples, max_features, idx_Mat, max_samples
):
    ...
    sample_indices = mp_seq_bts(idxM=idx_Mat, sample_len=max_samples, random_state=random_state)
...
def _parallel_build_estimators(n_estimators, ensemble, X, y, idx_Mat, sample_weight, seeds, total_n_estimators, verbose):
    ...
    for i in range(n_estimators):
        ...
        features, indices = _generate_bagging_indices(
            random_state, bootstrap_features, bootstrap, n_features, n_samples, max_features, idx_Mat, max_samples
        )
        ...
        if support_sample_weight:
            ...
            if bootstrap:
                ...
                not_indices_mask = ~_indices_to_mask(indices, n_samples)
    ...
    for estimator, features in zip(estimators, estimators_features):
        ...
        if n_classes == len(estimator.classes_):
            ...
            log_proba[:, estimator.classes_] = np.logaddexp(
                log_proba[:, estimator.classes_], log_proba_estimator[:, range(len(estimator.classes_))]
            )
...
class BaseBagging(BaseEnsemble, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        *,
        ...
        self.idx_Mat_Map = pd.Series(
            data=np.arange(self.idx_Mat.shape[1]), index=events.index
        )  # we need to reserve a copy of idx_matrix, if shuffle = True, idx_matrix will be mixed up.
    ...
    def _fit(self, X, y, max_samples=None, max_depth=None, sample_weight=None):
        ...
        self.estimators_bts_samples_ += list(
            itertools.chain.from_iterable(  # added something here, collect samples
    ...
    def _get_estimators_indices(self):
        ...
        for seed in self._seeds:
            ...
            feature_indices, sample_indices = _generate_bagging_indices(
                ...
                self.idx_Mat_,
                self._max_samples,
            )  # added something here, coud be this problem
...
class BaggingClassifier(ClassifierMixin, BaseBagging):
    ...
    @_if_delegate_has_method(delegate="base_estimator")
    def decision_function(self, X):
        ...
        all_decisions = Parallel(n_jobs=n_jobs, verbose=self.verbose)(
            delayed(_parallel_decision_function)(
                self.estimators_[starts[i] : starts[i + 1]], self.estimators_features_[starts[i] : starts[i + 1]], X
            )
            for i in range(n_jobs)
        )
