...
def tri_barrier(
    ...
    if side is None:
        side_, side, ptSl_ = 0, pd.Series(1.0, index=trgt.index), [ptSl[0], ptSl[0]]  # create side_ as counter
    else:
        side_, side, ptSl_ = 1, side.reindex(trgt.index), ptSl[:2]
    events = pd.concat({"t1": t1, "trgt": trgt, "side": side}, axis=1).dropna(subset=["trgt"])
    df0 = mp_pandas_obj(
        func=_pt_sl_t1, pd_obj=("molecule", events.index), num_threads=num_threads, data=data, events=events, ptSl=ptSl_
    )
...
def drop_label(events: pd.Series, min_pct: float = 0.05):
