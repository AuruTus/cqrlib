...
class RiskEstimators:
    ...
    def _find_max_eval(self, eigen_observations, tn_relation, kde_bwidth):
        ...
        optimization = minimize(
            self._pdf_fit, x0=np.array(0.5), args=(eigen_observations, tn_relation, kde_bwidth), bounds=((1e-5, 1 - 1e-5),)
        )
    ...
    def minimum_covariance_determinant(
        returns, price_data=False, assume_centered=False, support_fraction=None, random_state=None
    ):
        """
        ...
        cov_matrix = (
            MinCovDet(assume_centered=assume_centered, support_fraction=support_fraction, random_state=random_state)
            .fit(returns)
            .covariance_
        )
    ...
    @staticmethod
    def shrinked_covariance(returns, price_data=False, shrinkage_type="basic", assume_centered=False, basic_shrinkage=0.1):
        """
        ...
        if shrinkage_type == "basic":
            cov_matrix = ShrunkCovariance(assume_centered=assume_centered, shrinkage=basic_shrinkage).fit(returns).covariance_
