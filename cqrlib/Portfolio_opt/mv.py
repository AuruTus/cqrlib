...
class MV:
    ...
    def solve(
        ...
        expected_asset_returns, covariance = self._calculate_estimators(asset_prices, expected_asset_returns, covariance_matrix)
        ...
        if solution == "inverse_variance":
            ...
            self._min_volatility(covariance=covariance, expected_returns=expected_asset_returns)
        elif solution == "max_return_min_volatility":
            self._max_return_min_volatility(
                covariance=covariance, expected_returns=expected_asset_returns, risk_aversion=risk_aversion
            )
        elif solution == "max_sharpe":
            self._max_sharpe(covariance=covariance, expected_returns=expected_asset_returns)
        elif solution == "efficient_risk":
            self._min_volatility_for_target_return(
                covariance=covariance, expected_returns=expected_asset_returns, target_return=target_return
            )
        elif solution == "efficient_return":
            self._max_return_for_target_risk(
                covariance=covariance, expected_returns=expected_asset_returns, target_risk=target_risk
            )
        elif solution == "max_diversification":
            self._max_diversification(covariance=covariance, expected_returns=expected_asset_returns)
        else:
            self._max_decorrelation(covariance=covariance, expected_returns=expected_asset_returns)
    ...
    def plot_efficient_frontier(self, covariance, expected_asset_returns, min_return=0, max_return=0.4, risk_free_rate=0.05):
    ...
    def _error_checks(self, asset_names, asset_prices, expected_asset_returns, covariance_matrix, solution=None):
        ...
        if asset_prices is None and (expected_asset_returns is None or covariance_matrix is None):
            raise ValueError(
                "You need to supply either raw prices or expected returns and a covariance matrix of asset returns"
            )
    ...
    def _calculate_estimators(self, asset_prices, expected_asset_returns, covariance_matrix):
        ...
        if expected_asset_returns is None:
            if self.calculate_expected_returns == "mean":
                expected_asset_returns = self.returns_estimator.calculate_mean_historical_returns(asset_prices=asset_prices)
    ...
    def _min_volatility(self, covariance, expected_returns):
        ...
        if isinstance(self.weight_bounds, tuple):
            allocation_constraints.extend([weights >= self.weight_bounds[0], weights <= min(self.weight_bounds[1], 1)])
        else:
    ...
    def _max_sharpe(self, covariance, expected_returns):
        ...
        # Optimisation objective and constraints
        allocation_objective = cp.Minimize(risk)
        allocation_constraints = [cp.sum((expected_returns - self.risk_free_rate).T @ y) == 1, cp.sum(y) == kappa, kappa >= 0]
        if isinstance(self.weight_bounds, tuple):
            allocation_constraints.extend([y >= kappa * self.weight_bounds[0], y <= kappa * self.weight_bounds[1]])
        else:
            for inequality in self.weight_bounds:
                allocation_constraints.append(eval(inequality))
