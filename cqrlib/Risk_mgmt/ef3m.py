...
import pandas as pd
import random
from cqrlib.Util.multiprocess import process_jobs_, process_jobs
...
class M2N:
    ...
    def fit(self, mu2: list):
        ...
        while True:
            ...
            if error < self.error:
                self.param = param
                self.error = error
            if abs(p1 - param[4]) < self.epsilon:
                return  # threshold limitations
            if numIter > 1 / self.epsilon:
                return  # loop limitlations
            p1 = param[4]
    ...
    def get_mts(self, param: list):
        ...
        m3 = param[4] * (3 * param[2] ** 2 * param[0] + param[0] ** 3) + (1 - param[4]) * (
            3 * param[3] ** 2 * param[1] + param[1] ** 3
        )
        ...
        m5 = param[4] * (15 * param[2] ** 4 * param[0] + 10 * param[2] ** 2 * param[0] ** 3 + param[0] ** 5) + (
            1 - param[4]
    ...
    def iter4(self, mu2: float, p1: float):
        ...
        sig_2 = (
            (mts[2] + 2 * p1 * mu1**3 + (p1 - 1) * mu2**3 - 3 * mu1 * (mts[1] + mu2**2 * (p1 - 1)))
            / (3 * (1 - p1) * (mu2 - mu1))
        ) ** (0.5)

        sig_1 = ((mts[1] - sig_2**2 - mu2**2) / p1 + sig_2**2 + mu2**2 - mu1**2) ** (0.5)
    ...
    def iter5(self, mu2: float, p1: float):
        ...
        sig_1 = ((mts[1] - sig_2**2 - mu2**2) / p1 + sig_2**2 + mu2**2 - mu1**2) ** (0.5)
        a = (6 * sig_2**4 + (mts[3] - p1 * (3 * sig_1**4 + 6 * sig_1**2 * mu1**2 + mu1**4)) / (1 - p1)) ** 0.5
        mu2 = (a - 3 * sig_2**2) ** 0.5
...
def mts_fit(
    ...
    for i in np.arange(n_run):
        job = {"func": _mts_loop, "mts": mts, "epsilon": epsilon, "factor": factor, "variant": variant}
        jobs.append(job)
    if num_threads == 1:
