...
def avg_active_signals(signals: pd.DataFrame, num_threads: int = 1):
    ...
    t_pnts = list(t_pnts)
    t_pnts.sort()
    out = mp_pandas_obj(_avg_active_signals, ("molecule", t_pnts), num_threads=num_threads, signals=signals)
    return out


def discrete_signal(signal0: pd.DataFrame, step_size: float = 0.01):
    """
...
def _limit_price(target_pos, pos, forecast_price, width_coef, max_pos_size):
    ...
    for j in np.arange(abs(pos + sgn), abs(target_pos + 1)):
        limit_p += _inv_price(forecast_price=forecast_price, width_coef=width_coef, m_bet_size=j / float(max_pos_size))
...
def dynamic_bet(
    pos: float = 0,
    max_pos_size: float = 100.0,
    market_price: float = 100.0,
    forecast_price: float = 115.0,
    _w_params: dict = {"divergence": 10, "m_bet_size": 0.95},
):

    _wc = _width_coef(price_dvg=_w_params["divergence"], m_bet_size=_w_params["m_bet_size"])

    _trgt_pos = _target_position(
        width_coef=_wc, forecast_price=forecast_price, market_price=market_price, max_pos_size=max_pos_size
    )

    _lp = _limit_price(target_pos=_trgt_pos, pos=pos, forecast_price=forecast_price, width_coef=_wc, max_pos_size=max_pos_size)
...
def co_bets_size(
    ...
    df = pd.DataFrame(index=events.index).assign(t1=events["t1"], side=side)

    out = mp_pandas_obj(_co_position, ("molecule", events.index), num_threads=1, df=df, side=side)
    ...
    if budget:
        ...
        df0 = mts_fit(
            mts=r_mts, epsilon=epsilon, factor=factor, variant=variant, n_run=n_run, f_params=f_params, num_threads=num_threads
        )
...
def bet_size_power(width_coef, price_dvg):
    ...
    if not (-1 <= price_dvg <= 1):
        raise ValueError(f"Price divergence must be between -1 and 1, inclusive. Found price divergence value: {price_dvg}")
    if price_dvg == 0.0:
        return 0.0
