...
from cqrlib.Util.multiprocess import mp_pandas_obj, process_jobs_, process_jobs
...
def num_co_events(data: pd.Series, events: pd.DataFrame, num_threads: int):
    ...
    num_co_event = mp_pandas_obj(
        func=_num_co_events, pd_obj=("molecule", events.index), num_threads=num_threads, dataIndex=data.index, t1=events["t1"]
    )
...
def _mp_sample_TW(t1: pd.DatetimeIndex, num_conc_events: int, molecule):
    ...
    for tIn, tOut in t1.loc[wght.index].items():
        wght.loc[tIn] = (1.0 / num_conc_events.loc[tIn:tOut]).mean()
...
def wght_by_coevents(data: pd.Series, events: pd.DataFrame, num_threads: int):
    ...
    if isinstance(data, pd.Series):
        if isinstance(data.dtype, (str, dict, tuple, list)):
            raise ValueError(
                "data input must be pandas Series with dtype either float or integer value i.e. close price series"
            )
...
def _mp_idx_matrix(data: pd.DataFrame, molecule):
    ...
    """
    event_ = data[
        molecule.index.min() : molecule.max()
    ].index  # in the book they included 1 more date index as max not sure why
    indM_ = pd.DataFrame(0.0, index=event_, columns=np.arange(molecule.shape[0]))
    for i, (t0, t1) in enumerate(molecule.itertuples()):
        indM_.loc[t0:t1, i] = 1.0
...
def mp_idx_matrix(data: pd.Series, events: pd.DataFrame, num_threads: int = 1):
    ...
    indM = mp_pandas_obj(func=_mp_idx_matrix, pd_obj=("molecule", events.t1), num_threads=num_threads, axis=1, data=data)
...
def mp_seq_bts(idxM: pd.DataFrame, sample_len=None, num_threads=1, verbose: bool = False, random_state=np.random.RandomState()):
    """
    AFML pg 65 snippet 4.5
    This will make use of both av_unique and idx_matrix function to perform sequential bootstrap method.
    The current func includes both func for ease of use.
    ...
    while len(phi) < sample_len:
        avgU = pd.Series(0.0, dtype=float)
        for ind in idxM:
            jobs = []
            job = {"func": _mp_seq_bts, "avgU": avgU, "idxM": idxM, "phi": phi, "ind": ind, "sample_len": sample_len}
...
def seq_bts(idxM: pd.DataFrame, sample_len=None):
    ...
    while len(phi) < sample_len:
        ...
        prob = (
            avgU / avgU.sum()
        )  # this part lets you know the uniqueness of the entire series based on it's pick, hence assign prob
...
def MC_seq_bts(
    data: pd.DataFrame, events: pd.DataFrame, sample_len: int = None, num_iterations: int = 100, num_threads: int = 1
):
    """
    This is a modified func based on AFML pg 67 snippet 4.9
    A similar monte carlos simulation will be used to perform sequential bootstrap.
    ...
    for i in np.arange(int(num_iterations)):
        job = {"func": _mp_MC_seq_bts, "data": data, "events": events, "sample_len": sample_len}
        jobs.append(job)
        if num_threads == 1:
...
def MT_MC(num_obs: int = 10, num_bars: int = 100, max_H: int = 5, numIters: int = 1e6, num_threads: int = 24):
    ...
    """
    warnings.warn(
        "This func does not fit into other aglos, do not use it, it was created for easier understanding for mp.process_jobs!"
    )
...
def wght_by_rtn(data: pd.DataFrame, events: pd.DataFrame, num_threads: int = 1):
    ...
    num_co_events = mp_pandas_obj(
        _num_co_events, ("molecule", events.index), num_threads=num_threads, dataIndex=data.index, t1=events["t1"]
    )
...
def wght_by_td(data: pd.DataFrame, events: pd.DataFrame, num_threads: int = 1, td: float = 1.0):
    """
    ...
    av_un = wght_by_coevents(data=data.close, events=events, num_threads=num_threads)
