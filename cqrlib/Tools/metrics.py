...
from cqrlib.Tools.cross_validate import cv_score, PurgedKFold
    ...
    * Sklearn’s RandomForest class implements MDI as the default feature importance score. This choice is likely
    ...
    if clustered_subsets is not None:
        ...
        for subset in clustered_subsets:  # Iterating over each cluster
            ...
            importance.loc[subset, "std"] = subset_feat_imp.std() * subset_feat_imp.shape[0] ** -0.5
    else:
        importance = pd.concat(
            {"mean": feature_imp_df.mean(), "std": feature_imp_df.std() * feature_imp_df.shape[0] ** -0.5}, axis=1
        )
    ...
    * The CV must be purged and embargoed.
    ...
    for i, (train, test) in enumerate(cv_gen.split(X=X, y=y)):
        ...
        if scoring == "neg_log_loss":
            ...
            fold_metrics_values.loc[i] = -log_loss(y1, prob, sample_weight=w1.values, labels=classifier.classes_)
        else:
            pred = fit.predict(X1)
            fold_metrics_values.loc[i] = accuracy_score(y1, pred, sample_weight=w1.values)

        # Get feature specific metric on out-of-sample fold
        for j in X.columns:
            X1_ = X1.copy(deep=True)
            np.random.RandomState(seed=random_state).shuffle(
                X1_[j].values
            )  # Permutation of a single column for MDA shuffle values within column
            if scoring == "neg_log_loss":
                prob = fit.predict_proba(X1_)
                features_metrics_values.loc[i, j] = -log_loss(y1, prob, sample_weight=w1.values, labels=classifier.classes_)
            else:
                pred = fit.predict(X1_)
                features_metrics_values.loc[i, j] = accuracy_score(y1, pred, sample_weight=w1.values)
    ...
    importance = pd.concat({"mean": importance.mean(), "std": importance.std() * importance.shape[0] ** -0.5}, axis=1)
...
def sfi(
    ...
    * Like MDA, it can conclude that all features are unimportant, because performance is evaluated via OOS CV.
    ...
    for feat in feature_names:
        _cv_scores = cv_score(
            classifier=classifier, X=X[[feat]], y=y, sample_weight=sample_weight, scoring=scoring, cv_gen=cv_gen
        )

        importance.loc[feat, "mean"] = _cv_scores.mean()
        importance.loc[feat, "std"] = _cv_scores.std() * _cv_scores.shape[0] ** -0.5
    return importance
...
def plot_feat_imp(
    feat_imp_df: pd.DataFrame, oob_score: float, oos_score: float, method: str, output_path=None, save_fig: bool = False
):
    """
    ...
    if method == "MDI":
        plt.axvline(1.0 / feat_imp_df.shape[0], lw=1, c="r", ls="--")
    ax.get_yaxis().set_visible(False)
    for i, j in zip(ax.patches, feat_imp_df.index):
        ax.text(i.get_width() / 2, i.get_y() + i.get_height() / 2, j, ha="center", va="center", c="black")
...
def _feat_imp_analysis(
    ...
    if method == "MDI":
        imp = mdi(fitted_model=fit_model, train_features=X.columns)
...
def feat_imp_analysis(
    ...
    for _method in methods:
        feat_imp_score, oob_score, oos_score = _feat_imp_analysis(
            classifier=classifier,
            X=X,
            y=y,
            sample_weight=sample_weight,
