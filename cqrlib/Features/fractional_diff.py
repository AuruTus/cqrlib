...
def min_value(data: pd.Series, thres: float = 1e-2, pval_threshold: float = 0.05, num: int = 100, num_threads: int = 3):
    """
    ...
    d_domain = np.linspace(start=0, stop=1, num=num, endpoint=True, retstep=False, dtype=float)

    for d in d_domain:
        df1 = data.resample("1D").last()  # give a good estimate for time reduction.
        df2 = pd.Series()
        jobs = []
        job = {"func": fracDiff_FFD, "data": df1, "d": d, "thres": thres}
