...
def _opt_tr(molecule, n_run: float = 100, max_period: int = 100, seed: int = 0):

    out = pd.DataFrame(0.0, index=molecule.index, columns=["profit", "loss", "mean", "std", "sharpe"])
...
def opt_tr(
    profit_range: list = np.linspace(0.5, 10, 20),
    loss_range: list = np.linspace(0.5, 10, 20),
    sigma: list = [1],
    ...
    _coeff = dict(
        {"forecast": forecast, "half_life": half_life, "sigma": sigma, "profit_range": profit_range, "loss_range": loss_range}
    )
