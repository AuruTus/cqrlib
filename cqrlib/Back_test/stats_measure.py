...
from cqrlib.Util.multiprocess import mp_pandas_obj
    ...
    * res = weighted average time a trade was held
    ...
    for i in range(1, target_positions.size):
        ...
        if float(position_difference.iloc[i] * target_positions.iloc[i - 1]) >= 0:
            if float(target_positions.iloc[i]) != 0:  # And not an empty position
                entry_time = (
                    entry_time * target_positions.iloc[i - 1] + time_difference[i] * position_difference.iloc[i]
    ...
    if float(holding_period["weight"].sum()) > 0:  # If there were closed trades at all
        avg_holding_period = float(
            (holding_period["holding_time"] * holding_period["weight"]).sum() / holding_period["weight"].sum()
        )
    else:
...
def rtn_by_tw(mtx: pd.DataFrame, num_threads: int = 1):
    ...
    for idx in mtx.index:
        ...
        if ((_open_pos) > 0.0 and (_close_pos) >= 0.0) or ((_open_pos) < 0.0 and (_close_pos) >= 0.0):
...
def _hh_idx(rtn: pd.Series) -> float:
    ...
    hhi = (hhi - rtn.shape[0] ** (-1)) / (1.0 - rtn.shape[0] ** (-1))
...
def hh_idx(rtn: pd.Series, freq: str = "M"):
    ...
    * positive_concentration == 1 ⇔ only one non-zero return exists
    ...
    p("HHI positive: {0:.6f}\nHHI Negative: {1:.6f}\nHHI Time between: {2:.6f}\n".format(positive_hhi, negative_hhi, time_hhi))

    return (positive_hhi, negative_hhi, time_hhi)


def drawdn_period(data: pd.Series, dollars: bool = False, q_param: float = 0.0):
    """
    ...
    * Time under water index is the time of a high watermark and how much time
    ...
    time_under_water = np.append(time_under_water, (data.index[-1] - high_watermarks.index[-1]) / np.timedelta64(1, "Y"))

    time_under_water = pd.Series(time_under_water, index=high_watermarks.index)

    if q_param != 0.0 and dollars is False:
        p(
            "\nLoss Metric ({0} quartile)\n{1}\nDrawdown: {2:.6f}%\nTime under water (Based on 252 days): {3:.6f}\n".format(
                q_param, sep, draw_dn.quantile(q_param) * 100, time_under_water.quantile(q_param) * 252
            )
        )
    if q_param != 0.0 and dollars is not False:
        p(
            "\nLoss Metric ({0} quartile)\n{1}\nDrawdown: ${2:.2f}\nTime under water (Based on 252 days): {3:.6f}\n".format(
                q_param, sep, draw_dn.quantile(q_param), time_under_water.quantile(q_param) * 252
            )
        )
...
def sharpe_ratio(rtn: pd.Series, rf_param: float = 0.0, t_days: int = 252):
    """
...
def inform_ratio(rtn: pd.Series, benchmark: float = 0.0, t_days: int = 252):
    """
...
def proba_sr(obs_sr: float, benchmark_sr: float, num_returns: int, skew_returns: float = 0.0, kurt_returns: float = 3.0):
...
def deflated_sr(
    obs_sr: float, sr_est: list, num_returns: int, skew_returns: float = 0, kurt_returns: float = 3, est_param: bool = False
):

    # Calculating benchmark_SR from the parameters of estimates
    if est_param:
        benchmark_sr = sr_est[0] * (
            (1 - np.euler_gamma) * norm.ppf(1 - 1 / sr_est[1]) + np.euler_gamma * norm.ppf(1 - 1 / sr_est[1] * np.e ** (-1))
        )

    # Calculating benchmark_SR from a list of estimates
    else:
        benchmark_sr = np.array(sr_est).std() * (
            (1 - np.euler_gamma) * norm.ppf(1 - 1 / len(sr_est)) + np.euler_gamma * norm.ppf(1 - 1 / len(sr_est) * np.e ** (-1))
        )

    deflated_sr = proba_sr(
        obs_sr=obs_sr, benchmark_sr=benchmark_sr, num_returns=num_returns, skew_returns=skew_returns, kurt_returns=kurt_returns
    )

    return (deflated_sr, benchmark_sr)


def min_tr_len(obs_sr: float, benchmark_sr: float, skew_returns: float = 0, kurt_returns: float = 3, alpha: float = 0.05):
    """
    min track record length
    default to norma dist
    """

    tr_len = 1 + (1 - skew_returns * obs_sr + (kurt_returns - 1) / 4 * obs_sr**2) * (
        norm.ppf(1 - alpha) / (obs_sr - benchmark_sr)
    ) ** (2)
